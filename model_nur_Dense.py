...
from tensorflow.python.keras.layers import Activation, Dense, LSTM, Dropout, Lambda, Input, Multiply, Layer, Conv1D, LeakyReLU
...
class audio_generator():
    ...
    def count_samples(self):
        ...
        for file in self.file_names:
            ...
            self.total_samples = self.total_samples + np.fix((info.data.frame_count-(self.blockLen/2))/self.block_shift) -(self.len_of_samples-1)
    ...
    def create_generator(self):
     ...
     # check if training or validation
        ...
        for file in self.file_names:
            ...
            num_samples = STFT_noisyspeech_abs.shape[0] - (self.len_of_samples -1)

            for idx in range(num_samples):

                in_dat = STFT_noisyspeech_abs[idx:idx+self.len_of_samples,:]
                in_dat = in_dat.reshape(-1)
                tar_dat = tar[idx+self.len_of_samples-1,:]
    ...
    def create_tf_data_obj(self):
        self.tf_data_set = tf.data.Dataset.from_generator(
            ...
            output_shapes=(tf.TensorShape([257*6]), tf.TensorShape([257])),
            args=None
        )
...
class ModelTrain():
    ...
    def lossWrapper(self):
        ...
        #     loss = tf.math.multiply(tf.constant(5, dtype=tf.float32, shape=y_pred.shape),tf.math.abs(y_true-y_pred))
    ...
    def build_model(self, norm_stft = False):
        ...
        x = LeakyReLU(alpha=0.1)(x)
        x = Dropout(self.dropout)(x)
        x = Dense(257,activation='linear')(x)
